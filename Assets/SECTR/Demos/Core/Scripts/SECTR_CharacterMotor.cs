...
public class SECTR_CharacterMotor : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
		...
		if(MoveWithPlatform())
		{
			...
			if(yRotation != 0)
			{
				...
				cachedTransform.Rotate(0, yRotation, 0);
			}
		}
		...
		float pushDownOffset = Mathf.Max(cachedController.stepOffset, new Vector3(currentMovementOffset.x, 0, currentMovementOffset.z).magnitude);
		if(grounded)
		{
		...
		{
			movement.collisionFlags = cachedController.Move(currentMovementOffset);
		}
		...
		movement.velocity = (cachedTransform.position - lastPosition) / Time.deltaTime;
		...
		if(grounded && !IsGroundedTest())
		{
			...
			SendMessage("OnFall", movement.hitMaterial != null ? movement.hitMaterial : defaultHitMaterial, SendMessageOptions.DontRequireReceiver);
			// We pushed the character down to ensure it would stay on the ground ifthere was any.
			// But there wasn't so now we cancel the downwards offset to make the fall smoother.
			cachedTransform.position += pushDownOffset * Vector3.up;
		}
		...
		else if(!grounded && IsGroundedTest())
		{
			...
			SendMessage("OnLand", movement.hitMaterial != null ? movement.hitMaterial : defaultHitMaterial, SendMessageOptions.DontRequireReceiver);
		}
		...
		if(grounded && !TooSteep()) 
		{
			...
			{
				float footstepDistanceSqr = Vector3.SqrMagnitude(lastPosition - lastFootstepPosition);
				if(footstepDistanceSqr >= movement.footstepDistance * movement.footstepDistance)
				{
					SendMessage("OnFootstep", movement.hitMaterial != null ? movement.hitMaterial : defaultHitMaterial, SendMessageOptions.DontRequireReceiver); 
					lastFootstepPosition = lastPosition;
				}
			}
			else
			{
    ...
    {
        // Find desired velocity
        Vector3 desiredLocalDirection = cachedTransform.InverseTransformDirection(inputMoveDirection);
        ...
        return cachedTransform.TransformDirection(desiredLocalDirection * maxSpeed);
    }
    ...
    {
        return (groundNormal.y <= Mathf.Cos(cachedController.slopeLimit * Mathf.Deg2Rad));
    }
